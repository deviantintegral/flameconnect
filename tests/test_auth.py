...
class TestParseRedirectUrl:
    ...
    def test_error_without_description_raises(self):
        ...
        with pytest.raises(AuthenticationError, match="Auth error.*server_error"):
            MsalAuth._parse_redirect_url("https://redirect?error=server_error")
...
class TestMsalAuthEdgeCases:
    ...
    async def test_existing_cache_is_loaded(self, mock_msal, tmp_path):
        ...
        mock_app.acquire_token_silent.return_value = {"access_token": "loaded-token"}
        mock_msal.PublicClientApplication.return_value = mock_app
...
class TestBuildAppArgs:
    ...
    @patch("flameconnect.auth.msal")
    async def test_build_app_passes_client_id(self, mock_msal, tmp_path):
    ...
    @patch("flameconnect.auth.msal")
    async def test_build_app_cache_passed_as_token_cache(self, mock_msal, tmp_path):
...
class TestSilentAcquisitionArgs:
    ...
    @patch("flameconnect.auth.msal")
    async def test_silent_uses_scopes_and_first_account(self, mock_msal, tmp_path):
        ...
        mock_app.acquire_token_silent.assert_called_once_with(SCOPES, account=acct)

    @patch("flameconnect.auth.msal")
    async def test_silent_none_result_falls_through(self, mock_msal, tmp_path):
    ...
    @patch("flameconnect.auth.msal")
    async def test_silent_result_without_access_token_key(self, mock_msal, tmp_path):
...
class TestInteractiveFlowArgs:
    ...
    async def test_initiate_flow_receives_scopes_and_redirect(
        ...
        cache_path, _, mock_app, _ = self._setup_interactive_mocks(mock_msal, tmp_path)
    ...
    async def test_prompt_callback_receives_auth_and_redirect_uri(
        ...
        async def capturing_prompt(auth_uri: str, redirect_uri: str) -> str:
            received_args.append((auth_uri, redirect_uri))
            return "https://redirect?code=c1"

        auth = MsalAuth(cache_path=cache_path, prompt_callback=capturing_prompt)
        await auth.get_token()

        assert len(received_args) == 1
        assert received_args[0][0] == ("https://example.com/auth?foo=bar")
        assert received_args[0][1] == _REDIRECT_URI

    @patch("flameconnect.auth.msal")
    async def test_acquire_token_by_auth_code_flow_args(self, mock_msal, tmp_path):
        ...
        args = mock_app.acquire_token_by_auth_code_flow.call_args
    ...
    @patch("flameconnect.auth.msal")
    async def test_interactive_flow_saves_cache(self, mock_msal, tmp_path):
        """After successful interactive flow, cache is saved."""
        cache_path, mock_cache, mock_app, _ = self._setup_interactive_mocks(
            mock_msal, tmp_path
        )
    ...
    @patch("flameconnect.auth.msal")
    async def test_interactive_flow_strips_redirect_response(self, mock_msal, tmp_path):
    ...
    @patch("flameconnect.auth.msal")
    async def test_auth_uri_from_flow_used_in_prompt(self, mock_msal, tmp_path):
        ...
        mock_app.acquire_token_by_auth_code_flow.return_value = {"access_token": "t"}
        mock_msal.PublicClientApplication.return_value = mock_app
...
class TestTokenExchangeErrorDetails:
    ...
    @patch("flameconnect.auth.msal")
    async def test_exchange_error_defaults_when_missing(self, mock_msal, tmp_path):
...
class TestSaveCacheNotCalledWhenUnchanged:
    ...
    @patch("flameconnect.auth.msal")
    async def test_cache_not_written_when_unchanged(self, mock_msal, tmp_path):
...
class TestMsalAuthLogMessages:
    ...
    @patch("flameconnect.auth.msal")
    async def test_silent_acquisition_log_messages(self, mock_msal, tmp_path, caplog):
        ...
        assert any(m == "Attempting silent token acquisition" for m in messages)
        assert any(m.startswith("Token acquired silently") for m in messages)

    @patch("flameconnect.auth.msal")
    async def test_interactive_flow_log_messages(self, mock_msal, tmp_path, caplog):
        ...
        mock_app.acquire_token_by_auth_code_flow.return_value = {"access_token": "tok"}
        mock_msal.PublicClientApplication.return_value = mock_app
        ...
        messages = [r.message for r in caplog.records]
        assert any(m.startswith("No cached token") for m in messages)
        assert any(m.startswith("Exchanging authorization code") for m in messages)
        assert any(m.startswith("Authentication successful") for m in messages)

    @patch("flameconnect.auth.msal")
    async def test_save_cache_log_message(self, mock_msal, tmp_path, caplog):
        ...
        messages = [r.message for r in caplog.records]
        assert any(m.startswith("Token cache saved to") for m in messages)
        # Verify the cache path is included in the log message
        assert any(str(cache_path) in m for m in messages)
